...
const MovieDetailsPage = () => {
  ...
  const { backdrop_path, title, genres } = data || [];
  console.log(data);
  return (
    <>
      ...
      {genres?.length > 0 && (
        <div className="flex items-center gap-x-10 mb-5 w-[60%] mx-auto text-center">
          {genres?.map((item, index) => (
            <span
              key={index}
              className="px-5 py-2 rounded-3xl border border-primary"
            >
              {item.name}
            </span>
          ))}
